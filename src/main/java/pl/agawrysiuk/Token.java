...
public class Token extends ViewCard {
    ...
    public Token(int attack, int defense, Color color,String type, String additionalText) {
        ...
        attDef.setFont(Font.font(Font.getDefault().getName(), FontWeight.BOLD, 120*StartWindowController.X_WINDOW));
        attDef.setEffect(new DropShadow(BlurType.GAUSSIAN, Color.rgb(1, 1, 1, 1), 4, 0.9, 0, 0));
        attDef.setText(attack + " / " + defense);
        StackPane.setMargin(attDef,new Insets(-270*StartWindowController.X_WINDOW,0,0,0));
        sPane.getChildren().add(attDef);

        Text addText = new Text();
        addText.prefWidth(400*StartWindowController.X_WINDOW);
        addText.setWrappingWidth(350*StartWindowController.X_WINDOW);
        addText.setTextAlignment(TextAlignment.LEFT);
        addText.setFill(Color.WHITE);
        addText.setFont(Font.font(Font.getDefault().getName(), FontWeight.BOLD, 20*StartWindowController.X_WINDOW));
        addText.setEffect(new DropShadow(BlurType.GAUSSIAN, Color.rgb(1, 1, 1, 1), 4, 0.9, 0, 0));
        addText.setText(additionalText);
        StackPane.setMargin(addText,new Insets(160*StartWindowController.X_WINDOW,0,0,0));
        ...
        BufferedImage artImg = (SwingFXUtils.fromFXImage(this.cardImg, null))
                .getSubimage((int) (25*StartWindowController.X_WINDOW), (int) (25*StartWindowController.X_WINDOW), (int) (430*StartWindowController.X_WINDOW), (int) (357*StartWindowController.X_WINDOW)); //15,15,450,370
